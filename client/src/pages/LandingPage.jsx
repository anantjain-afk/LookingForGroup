                ...
                <div className="space-y-12">
                     ...
                     {user.favoriteGames && user.favoriteGames.length > 0 && (
                         <div className="space-y-6">
                            <h2 className="text-2xl font-bold text-white mb-6">Your Favorite Games</h2>
                            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                              {user.favoriteGames.map((game) => (
                                <div 
                                  key={game.id} 
                                  onClick={() => navigate(`/game/${game.igdbId || game.id}`)}
                                  className="group relative bg-[#151515] rounded-xl overflow-hidden cursor-pointer hover:scale-105 transition-all duration-300 border border-transparent hover:border-[#5865F2] hover:shadow-[0_0_15px_rgba(88,101,242,0.2)]"
                                >
                                  {/* Image Container */}
                                  <div className="aspect-[3/4] w-full relative">
                                    <img 
                                      src={game.imageUrl || "https://placehold.co/300x400/1e293b/475569?text=No+Cover"} 
                                      alt={game.name} 
                                      className="w-full h-full object-cover group-hover:opacity-80 transition-opacity"
                                      loading="lazy"
                                    />
                                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-4">
                                      <span className="text-[#5865F2] text-xs font-bold uppercase tracking-wider mb-1">View Lobbies</span>
                                    </div>
                                  </div>
                                  
                                  {/* Game Info */}
                                  <div className="p-3">
                                    <h3 className="text-white font-bold truncate text-sm md:text-base group-hover:text-[#5865F2] transition-colors">{game.name}</h3>
                                    {game.genre && (
                                      <p className="text-gray-500 text-xs truncate mt-1">{game.genre}</p>
                                    )}
                                  </div>
                                </div>
                              ))}
